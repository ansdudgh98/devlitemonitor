...
const GrafanaOrgAccountCreatePage = () => {
    ...
    const [grafanaOrg, setGrafanaOrg] = useState("");
    ...
    const [validationGrafanaOrg, setValidationGrafanaOrg] = useState(false);
    ...
    const handleInputGrafanaOrg = e => {
        setGrafanaOrg(e.target.value);

        if (!lengthValidation(e)) {
            setValidationGrafanaOrg(false);
            return;

        }
        setValidationGrafanaOrg(true);
    ...
    const accountCreateForm = e => {
        ...
        if(!validationGrafanaId || !validationGrafanaPw || !validationGrafanaEmail || !validationGrafanaOrg || !validationGrafanaName) {
            alert("양식을 지켜주세요!");
    ...
    const accountCreateAPI = async () => {
        await axiosInstance.post("/grafana/createOrg",{
            "grafanaId": grafanaId,
            "grafanaPw": grafanaPw,
        ...
        <div className="container-xxl">
            ...
            <div className="vstack gap-xxl-5 rounded border border-primary">
                ...
                <form id="accountForm">
                    ...
                    <input type={"text"} className={`control rounded border`} id={"grafanaOrg"} onChange={handleInputGrafanaOrg} placeholder="grafana org"/>
                    {
                            <label
                                style={{
                                    visibility : `${errorStyle ? 'visible' : 'hidden' }`,
